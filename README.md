...
- **Performance is workload- and host-specific.** New arm64 Release measurements: 9.69M rec/s median at N=2 (7 reps, 1M records, 1000 keys), versus 5.85M single-threaded on the same workload; 9.44–9.55M rec/s in the 20K-pane isolation benchmark. On the [matched 100M-record Flink comparison](bench/flink/README.md#reference-result-apple-m1-max), stormglass reached 9.07M rec/s at N=1 and 30.02M at N=4, 3.87–4.62x the Flink 2.3.0 medians across N in {1,2,4,8}, with identical output counts and digests. Historical 44–56% overhead reduction has not been independently reproduced by this audit.
